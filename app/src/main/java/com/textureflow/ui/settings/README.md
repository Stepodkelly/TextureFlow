...
| `IntelligenceSection.java` | Mode toggle, Gemma download, §9.4 wizard, ledger + role-run metrics, teal-pip legend |
| `IntelligencePreferences.java` | `texture_surface_preferences` / `intelligence_mode` (default on) |
| `SettingsMetrics.java` | `runtime.ledger()` assessments and role runs (instanceof + inspectors) |
...
“Test this phone” runs a 600-token prefill + 64-token decode, then a short draft generate, and stores `CapabilityProfile.Bench` on the ledger.
